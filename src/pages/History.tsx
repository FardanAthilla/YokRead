...
const History = () => {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      console.log(
        "📢 [Auth] onAuthStateChanged terpanggil:",
        u ? u.uid : "Tidak login"
      );
...
useEffect(() => {
  const fetchComics = async () => {
    ...
    console.log("🧩 [Fetch] Mulai ambil detail komik, total:", keys.length);
    ...
    try {
      const limit = pLimit(5); // 🚀 Batasi hanya 5 fetch berjalan bersamaan
      ...
      const fetchPromises = keys.map((key) =>
        limit(async () => {
          try {
            ...
            console.log(`✅ [Fetch] Berhasil ambil: ${comic.title}`);
            ...
            console.error(`❌ [Fetch] Gagal memuat komik ${key}:`, err);
            return null; // supaya tetap lanjut fetch lainnya
          }
        })
      );
      ...
      // 🔽 Urutkan descending (yang terakhir dibaca tampil duluan)
      setComics(validResults.reverse());
      console.log("✅ [Fetch] Selesai ambil semua komik:", validResults);
    } catch (err) {
      console.error("💥 [Fetch] Terjadi error umum:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  // 🧭 Loading Screen (tetap tampilkan Navbar)
  if (loading)
    return (
      <div className="bg-[#171717] text-white min-h-screen flex flex-col">
        <Navbar /> {/* ✅ Navbar tetap muncul saat loading */}
