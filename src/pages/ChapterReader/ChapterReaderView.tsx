...
const ChapterReaderView = ({
  ...
  if (loading || pages.length === 0) {
    return (
      <div className="flex justify-center items-center h-screen bg-[#171717] text-white relative overflow-hidden">
        <div className="fixed top-0 left-0 w-full z-[9999] backdrop-blur-md p-3 flex items-center text-white">
          <button
            onClick={handleBack}
  ...
  return (
    <div className="relative w-full md:max-w-3xl md:mx-auto md:p-4 min-h-screen text-white" onClick={handleToggleNavbar}>
      {/* Navbar atas */}
      <div
        className={`fixed top-0 left-0 w-full z-[9999] bg-[#171717]/90  backdrop-blur-md p-3 flex items-center text-white transition-transform duration-300 ${
      ...
      {/* Tombol bawah */}
      <div
        className={`fixed bottom-0 right-0 w-full bg-[#171717]/90 backdrop-blur-md text-white flex justify-end items-center px-4 py-3 transition-transform duration-300 ${
          showNavbar ? "translate-y-0" : "translate-y-full"
        }`}
      >
